...
public class SimpleOpponent extends Player implements Opponent  {
	...
	public SimpleOpponent(int playerIdx) {
	...
	public SimpleOpponent(int playerIdx, int difficulty) {
		super(null, playerIdx);
	...
	@Override
	public Move getNexMove( GameState state, Move lastPlayedMove) { // regularan potez
	...
	private int MinMax(int player, int depth) {
		...
		if (possibleMoves.isEmpty()) {
			return (player == Opponent.PLAYER_MAX) ? Opponent.MINUS_INFINITY + 1 : Opponent.INFINITY - 1;
		}

		int bestEstimation = (player == Opponent.PLAYER_MAX) ? Opponent.MINUS_INFINITY : Opponent.INFINITY;
		...
		for (Move move : possibleMoves) {
			...
			Move temp = lastMove;
			lastMove = move;
			int stateEstimation = MinMax((player == Opponent.PLAYER_MAX) ? Opponent.PLAYER_MIN : Opponent.PLAYER_MAX,
					depth + 1);
		...
		for (int i = 0; i < possibleMovesFigure1.size(); i++) {
			...
			&& (levels[index] == levels[fig1[0]] || (levels[index]  == levels[fig1[0]] + 1)) ) {
				...
				for (int j = 0; j < possibleTileInc.size(); j++) {
					...
					&& (tileIndex != fig1[1]) && (tileIndex != fig1[1])) {
						Move newMove = new Move(0, fig1[1], index, tileIndex);
